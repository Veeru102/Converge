...
- **H4** Re-timestamping is done at the `Welcome` boundary, before the pending flush, over the *entire* pending queue, in counter order, using the corrected clock. Never per-op mid-stream.
