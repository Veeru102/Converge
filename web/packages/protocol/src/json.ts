...
export interface PresenceJson {
  cursor: [number, number] | null;
  selection: IdJson[];
}
export interface PresenceEntryJson {
  replica: string;
  user: UserJson;
  state: PresenceJson;
}
...
export function presenceFromJson(j: PresenceJson): PresenceState {
  return {
    cursor: Array.isArray(j.cursor) ? [j.cursor[0], j.cursor[1]] : null,
    selection: (j.selection ?? []).map(idFromJson),
  };
}
function entryToJson(e: PresenceEntry): PresenceEntryJson {
  return {
    replica: e.replica.toString(),
    user: userToJson(e.user),
    state: presenceToJson(e.state),
  };
}
function entryFromJson(j: PresenceEntryJson): PresenceEntry {
  return {
    replica: BigInt(j.replica),
    user: userFromJson(j.user),
    state: presenceFromJson(j.state),
  };
}
  ...
  switch (m.type) {
    case "hello":
      return {
        ...
        doc: m.hello.doc,
        replica: m.hello.replica.toString(),
        last_seq: m.hello.lastSeq.toString(),
        want_snapshot: m.hello.wantSnapshot,
        user: userToJson(m.hello.user),
      };
    case "submit":
    ...
    case "hello":
      return {
        ...
        hello: {
          ...
          doc: j.doc,
          replica: BigInt(j.replica),
          lastSeq: BigInt(j.last_seq),
          wantSnapshot: !!j.want_snapshot,
          user: userFromJson(j.user),
        },
      };
    case "submit":
...
export type CatchUpJson =
  { ops: Array<{ seq: string; op: OpJson }> } | { snapshot: string; seq: string };
export type ServerMsgJson =
  | {
      type: "welcome";
      session: string;
      server_time_ms: string;
      durable_head_seq: string;
      catch_up: CatchUpJson;
      presence: PresenceEntryJson[];
    }
...
function catchUpFromJson(j: CatchUpJson): CatchUp {
  if ("ops" in j)
    return { kind: "ops", ops: j.ops.map((e) => ({ seq: BigInt(e.seq), op: opFromJson(e.op) })) };
  ...
  switch (m.type) {
    case "welcome":
      return {
        ...
        server_time_ms: m.serverTimeMs.toString(),
        durable_head_seq: m.durableHeadSeq.toString(),
        catch_up: catchUpToJson(m.catchUp),
        presence: m.presence.map(entryToJson),
      };
    case "bye":
      return { type: "bye", reason: m.reason };
    case "commit":
      return { type: "commit", seq: m.seq.toString(), op: opToJson(m.op) };
    case "ack":
      return {
        type: "ack",
        op_id: idToJson(m.opId),
        seq: m.seq === null ? null : m.seq.toString(),
      };
    case "nack":
      return { type: "nack", op_id: idToJson(m.opId), reason: m.reason };
    case "presence_update":
      return { type: "presence_update", entry: entryToJson(m.entry) };
    case "presence_leave":
      return { type: "presence_leave", replica: m.replica.toString() };
    case "ping":
      return { type: "ping", nonce: m.nonce.toString() };
  }
}
...
export function serverMsgFromJson(j: ServerMsgJson): ServerMsg {
  switch (j.type) {
    case "welcome":
      return {
        type: "welcome",
        session: BigInt(j.session),
        serverTimeMs: BigInt(j.server_time_ms),
        durableHeadSeq: BigInt(j.durable_head_seq),
        catchUp: catchUpFromJson(j.catch_up),
        presence: (j.presence ?? []).map(entryFromJson),
      };
    case "bye":
      return { type: "bye", reason: j.reason };
    case "commit":
      return { type: "commit", seq: BigInt(j.seq), op: opFromJson(j.op) };
    case "ack":
      return {
        type: "ack",
        opId: idFromJson(j.op_id),
        seq: j.seq === null || j.seq === undefined ? null : BigInt(j.seq),
      };
    case "nack":
      return { type: "nack", opId: idFromJson(j.op_id), reason: j.reason };
    case "presence_update":
      return { type: "presence_update", entry: entryFromJson(j.entry) };
    case "presence_leave":
      return { type: "presence_leave", replica: BigInt(j.replica) };
    case "ping":
      return { type: "ping", nonce: BigInt(j.nonce) };
  }
}
