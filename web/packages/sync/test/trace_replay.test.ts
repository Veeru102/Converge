...
import { hex, hlcFromJson, opFromJson, opToJson, unhex, type OpJson, type HlcJson } from "@converge/engine";
import { clientMsgToJson, serverMsgFromJson, userFromJson, type ServerMsgJson, type UserJson } from "@converge/protocol";
import { SyncEngine, type Now, type Output } from "../src/engine.js";

const DIR = join(import.meta.dirname, "../../../../fixtures/sync");

interface NowJson { wall: string; mono: string }
type StepIn =
  ...
  | { kind: "snapshot"; expect: { seq: string; high_water: HlcJson; acked: string[]; snapshot_hash: string } }
  | { kind: "snapshot_persisted"; acked: string[] }
  | { kind: "message"; gen: string; now: NowJson; msg: ServerMsgJson }
  | { kind: "restore"; row: { next_counter: string; high_water: HlcJson; clock_offset_ms: string }; snapshot: { bytes: string; seq: string } | null; pending: OpJson[] };
interface Step { in: StepIn; out: unknown[]; hash: string; last_seq: string; pending: Array<[string, boolean, boolean, boolean]> }
interface Trace {
  scenario: string; seed: number; client: number; doc: string; replica: string; user: UserJson;
  config: { retimestamp_margin_ms: number; hello_timeout_ms: number; ack_timeout_ms: number };
  steps: Step[]; final_hash: string;
}
...
function outJson(out: Output[]): unknown[] {
  return out.map((o) => {
    switch (o.type) {
      case "send": return { send: clientMsgToJson(o.msg) };
      case "persist": return { persist: opToJson(o.op) };
...
describe("SyncEngine replays the Rust reference traces", () => {
  ...
  for (const file of files) {
    it(file, () => {
      ...
      const cfg = { retimestampMarginMs: BigInt(t.config.retimestamp_margin_ms), helloTimeoutMs: t.config.hello_timeout_ms, ackTimeoutMs: t.config.ack_timeout_ms };
      ...
      t.steps.forEach((step, i) => {
        ...
        switch (s.kind) {
          case "connected": engine.connected(now(s.now), out); break;
          case "disconnected": engine.disconnected(); break;
          case "tick": engine.tick(now(s.now), out); break;
          case "edit": {
          ...
          case "persisted": engine.persisted(BigInt(s.counter), now(s.now), out); break;
          case "snapshot": {
          ...
          case "snapshot_persisted": engine.snapshotPersisted(s.acked.map(BigInt)); break;
          case "message": engine.message(Number(s.gen), serverMsgFromJson(s.msg), now(s.now), out); break;
          case "restore":
            engine = SyncEngine.restore(
              t.doc, BigInt(t.replica), userFromJson(t.user), cfg,
              { nextCounter: BigInt(s.row.next_counter), highWater: hlcFromJson(s.row.high_water), clockOffsetMs: BigInt(s.row.clock_offset_ms) },
        ...
        expect(engine.pendingDebug().map(([c, d, se, a]) => [c.toString(), d, se, a]), where).toEqual(step.pending);
      });
