...
export async function newDocId(request: {
  post: (url: string) => Promise<{ json: () => Promise<unknown> }>;
}): Promise<string> {
    ...
    .poll(
      async () => {
        ...
        const states = await Promise.all(
          pages.map(async (p) => ({ hash: await hash(p), st: await status(p) })),
        );
        const ok =
          ...
          states.every(
            (s) =>
              s.hash === server.hash &&
              s.st.unacked === 0 &&
              s.st.lastSeq === String(server.durable_seq),
          );
...
export async function addRects(
  page: Page,
  n: number,
  origin: [number, number] = [120, 120],
): Promise<void> {
  for (let i = 0; i < n; i++) {
    await page.getByTestId("tool-rect").click();
    const x = origin[0] + (i % 5) * 150,
      y = origin[1] + Math.floor(i / 5) * 120;
...
export async function dragFirstShape(page: Page, dx: number, dy: number): Promise<void> {
  ...
  for (let i = 1; i <= 8; i++)
    await page.mouse.move(
      box.x + box.width / 2 + (dx * i) / 8,
      box.y + box.height / 2 + (dy * i) / 8,
    );
  await page.mouse.up();
}
...
export async function setChaos(baseURL: string, cfg: Record<string, unknown>): Promise<void> {
  const res = await fetch(`${baseURL}/admin/chaos`, {
    method: "PUT",
