...
const kinds = (page: Page) =>
  page.getByTestId("shape").evaluateAll((els) => els.map((e) => e.getAttribute("data-kind")));
...
test("every tool draws, and both tabs render the same objects", async ({
  browser,
  request,
  baseURL,
}) => {
...
test("marquee multi-select moves several objects and shows in the other tab's presence", async ({
  browser,
  request,
  baseURL,
}) => {
  ...
  await expect
    .poll(() => b.page.locator("svg.canvas g[pointer-events=none] rect[stroke]").count())
    .toBeGreaterThanOrEqual(2);
  const xs = async () =>
    a.page.getByTestId("shape").evaluateAll((els) => els.map((e) => e.getBoundingClientRect().x));
...
test("network lab: presets drive server chaos, offline toggle queues ops, badge returns to converged", async ({
  browser,
  request,
  baseURL,
}) => {
  ...
  await expect
    .poll(
      async () =>
        (
          (await (await fetch(`${baseURL}/admin/chaos`)).json()) as {
            enabled: boolean;
            drop_p: number;
          }
        ).drop_p,
  ...
  await expect
    .poll(
      async () =>
        ((await (await fetch(`${baseURL}/admin/chaos`)).json()) as { enabled: boolean }).enabled,
    )
    .toBe(false);
  ...
  await expect(a.page.getByTestId("bench-result")).toContainText("200 ops committed", {
    timeout: 30_000,
  });
  await expect(a.page.getByTestId("bench-latency")).not.toContainText("—");
  await a.page.getByTestId("bench-offline-burst").click();
  await expect(a.page.getByTestId("bench-result")).toContainText("queued offline and committed", {
    timeout: 30_000,
  });
  await expectConverged([a.page], baseURL!, doc);
  await a.context.close();
