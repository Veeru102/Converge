...
export const toWorld = (v: Viewport, sx: number, sy: number) => ({
  x: (sx - v.x) / v.scale,
  y: (sy - v.y) / v.scale,
});
...
export function fitTo(
  ...
  const scale = Math.min(
    MAX_SCALE,
    Math.max(MIN_SCALE, Math.min((width - pad * 2) / box.w, (height - pad * 2) / box.h, 2)),
  );
  return {
    scale,
    x: width / 2 - (box.x + box.w / 2) * scale,
    y: height / 2 - (box.y + box.h / 2) * scale,
  };
}
