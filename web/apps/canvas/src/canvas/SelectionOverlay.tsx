...
export function handlePoint(b: Box, h: Handle): [number, number] {
  ...
  switch (h) {
    case "nw": return [b.x, b.y];
    case "n": return [cx, b.y];
    case "ne": return [b.x + b.w, b.y];
    case "e": return [b.x + b.w, cy];
    case "se": return [b.x + b.w, b.y + b.h];
    case "s": return [cx, b.y + b.h];
    case "sw": return [b.x, b.y + b.h];
...
export function SelectionOverlay({ selected, scale, onHandleDown, onLineEndDown }: Props) {
    ...
    return (
      <g data-testid="selection">
        ...
        {handles.map((h) => {
          ...
          return <rect key={h} x={x - hs / 2} y={y - hs / 2} width={hs} height={hs} fill="#fff" stroke={stroke} strokeWidth={1.5 * s} style={{ cursor: CURSORS[h] }} onPointerDown={(e) => onHandleDown(e, h)} data-handle={h} />;
        })}
      </g>
    );
  }
  ...
  return (
    <g data-testid="selection" pointerEvents="none">
      {selected.map((o, i) => {
        const b = bounds(o);
        return <rect key={i} x={b.x} y={b.y} width={b.w} height={b.h} fill="none" stroke={stroke} strokeWidth={1 * s} opacity={0.6} />;
      })}
      <rect x={all.x} y={all.y} width={all.w} height={all.h} fill="none" stroke={stroke} strokeWidth={1.5 * s} strokeDasharray={`${4 * s} ${3 * s}`} />
    </g>
  );
}
