...
export const ShapeView = memo(function ShapeView({ id, o, interactive, editing, onPointerDown, onDoubleClick }: Props) {
  ...
  switch (o.kind) {
    case "ellipse": {
      const x = num(o, "x"), y = num(o, "y"), w = num(o, "w", 100), h = num(o, "h", 60);
      return (
        <g {...common}>
          <ellipse cx={x + w / 2} cy={y + h / 2} rx={w / 2} ry={h / 2} fill={hexOf(color(o, "fill"), "#c7d2fe")} stroke={hexOf(color(o, "stroke"), "none")} strokeWidth={num(o, "stroke_width", 0)} />
        </g>
      );
    }
    case "line": {
      ...
      return (
        <g {...common}>
          <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="transparent" strokeWidth={Math.max(12, sw + 8)} />
          <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={stroke} strokeWidth={sw} strokeLinecap="round" />
          {bool(o, "arrow") && <polygon points={arrowHead(x1, y1, x2, y2, 6 + sw * 2)} fill={stroke} />}
        </g>
      );
    }
    case "text": {
      ...
      return (
        <g {...common}>
          ...
          {!editing && (
            <text x={box.x} y={box.y} fontSize={size} fill={fill} fontFamily="Inter, system-ui, sans-serif" style={{ whiteSpace: "pre" }}>
              {lines.map((l, i) => (
                <tspan key={i} x={box.x} y={box.y + size * 0.8 + i * size * TEXT_LINE}>{l || " "}</tspan>
              ))}
            </text>
          )}
        </g>
    ...
    default: {
      ...
      return (
        <g {...common}>
          <rect x={x} y={y} width={w} height={h} rx={3} fill={hexOf(color(o, "fill"), "#c7d2fe")} stroke={hexOf(color(o, "stroke"), "none")} strokeWidth={num(o, "stroke_width", 0)} />
        </g>
      );
    }
  }
