...
export function RemoteSelections({ presence, doc, scale }: { presence: PresenceEntry[]; doc: Document; scale: number }) {
  const s = 1 / scale;
  return (
        ...
        p.state.selection.map((id) => {
          ...
          return (
            <g key={`${p.replica}-${idKey(id)}`}>
              <rect x={b.x} y={b.y} width={b.w} height={b.h} fill="none" stroke={c} strokeWidth={1.5 * s} />
              <g transform={`translate(${b.x} ${b.y - 18 * s}) scale(${s})`}>
                <rect width={p.user.name.length * 6.6 + 10} height={16} rx={3} fill={c} />
                <text x={5} y={11.5} fontSize={11} fill="#fff" fontFamily="Inter, system-ui, sans-serif" fontWeight={500}>{p.user.name}</text>
              </g>
            </g>
          );
        }),
...
export function RemoteCursors({ presence, scale }: { presence: PresenceEntry[]; scale: number }) {
  ...
  for (const p of presence) {
    if (p.state.cursor) targets.current.set(p.replica.toString(), { x: p.state.cursor[0], y: p.state.cursor[1] });
  }
  ...
  useEffect(() => {
    ...
    const step = () => {
      for (const [k, t] of targets.current) {
        ...
        const nx = cur.x + (t.x - cur.x) * 0.35, ny = cur.y + (t.y - cur.y) * 0.35;
        shown.current.set(k, { x: nx, y: ny });
        nodes.current.get(k)?.setAttribute("transform", `translate(${nx} ${ny}) scale(${1 / scale})`);
      }
    ...
    <g pointerEvents="none">
      {presence.map((p) => {
        ...
        return (
          ...
          >
            <path d="M0 0 L0 16 L4.5 12.5 L8 19 L10.5 18 L7 11.5 L12.5 11.5 Z" fill={c} stroke="#fff" strokeWidth={1} />
            <rect x={13} y={12} width={p.user.name.length * 6.6 + 10} height={16} rx={3} fill={c} />
            <text x={18} y={23.5} fontSize={11} fill="#fff" fontFamily="Inter, system-ui, sans-serif" fontWeight={500}>{p.user.name}</text>
          </g>
        );
      })}
    </g>
