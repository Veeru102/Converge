...
import { createShape, deleteMany, moveTo, setGeometry, setLineEnd, setText, type Style, type Tool } from "../state/actions.js";
...
type Mode =
  ...
  | { t: "drag"; items: Array<{ id: ObjectId; o: ObjectState; origin: Box }>; x0: number; y0: number; moved: boolean; last: number; sent: string }
  | { t: "resize"; id: ObjectId; o: ObjectState; handle: Handle; origin: Box; x0: number; y0: number; last: number; sent: string }
  | { t: "lineend"; id: ObjectId; which: 1 | 2; last: number; sent: string }
  | { t: "draw"; kind: Exclude<Tool, "select">; id: ObjectId | null; x0: number; y0: number; last: number; sent: string };
...
export function CanvasView(p: CanvasProps) {
  const { client, objects, presence, tool, selection, setSelection, viewport, setViewport, editingId, setEditingId, spaceHeld } = p;
  ...
  const selectedEntries = useMemo(() => [...selection].map((k) => byKey.get(k)).filter((e): e is Entry => !!e), [selection, byKey]);
  ...
  const onShapeDown = (e: React.PointerEvent, id: ObjectId) => {
    ...
    const w = world(e);
    const items = [...next].map((k) => byKey.get(k)).filter((x): x is Entry => !!x).map(([iid, o]) => ({ id: iid, o, origin: bounds(o) }));
  ...
  const onHandleDown = (e: React.PointerEvent, handle: Handle) => {
    ...
    const w = world(e);
    mode.current = { t: "resize", id, o, handle, origin: bounds(o), x0: w.x, y0: w.y, last: 0, sent: "" };
  };
  ...
  const resized = (m: Extract<Mode, { t: "resize" }>, wx: number, wy: number, keepAspect: boolean): Box => {
    const { origin: b, handle } = m;
    const dx = wx - m.x0, dy = wy - m.y0;
    ...
    if (keepAspect && handle.length === 2 && b.w > 0 && b.h > 0) {
      ...
      const nw = b.w * k * Math.sign(w || 1), nh = b.h * k * Math.sign(h || 1);
  ...
  const step = (e: React.PointerEvent, final: boolean) => {
    ...
    switch (m.t) {
      ...
      case "draw": {
        ...
        const w = world(e);
        const box = m.kind === "line" ? { x: m.x0, y: m.y0, w: w.x - m.x0, h: w.y - m.y0 } : normBox(m.x0, m.y0, w.x, w.y);
        if (m.id === null) {
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    switch (m.t) {
      ...
      case "marquee": {
        ...
        if (box.w > 1 || box.h > 1) {
          const hit = objects.filter(([, o]) => intersects(box, bounds(o))).map(([id]) => idKey(id));
      ...
      case "draw": {
        if (m.kind === "text") {
          ...
          const id = createShape(client, "text", { x: m.x0, y: m.y0 - 11, w: 0, h: 0 }, p.style, p.textColor);
          setSelection(new Set([idKey(id)]));
          setEditingId(id);
        } else if (m.id === null) {
          const size = m.kind === "line" ? { x: m.x0, y: m.y0, w: 140, h: 0 } : { x: m.x0 - 60, y: m.y0 - 40, w: 120, h: 80 };
  ...
  const textThrottle = useRef<{ timer: ReturnType<typeof setTimeout> | null; pending: string | null; last: number }>({ timer: null, pending: null, last: 0 });
  ...
  const cls = ["canvas", panning ? "panning" : spaceHeld ? "space" : tool === "select" ? "tool-select" : "tool-draw"].join(" ");
  ...
  return (
    ...
    >
      <defs>
        <pattern id="grid" width={gridSize} height={gridSize} patternUnits="userSpaceOnUse" x={viewport.x % gridSize} y={viewport.y % gridSize}>
