...
const ms = (v: number | null | undefined) =>
  v === null || v === undefined ? "—" : v < 10 ? v.toFixed(1) : Math.round(v).toString();
...
export function BenchPanel({ client }: { client: Client }) {
  ...
  const burst = async (n: number, offlineFirst: boolean) => {
    ...
    const t0 = performance.now();
    for (let i = 0; i < n; i++)
      client.edit({ op: "set_props", object: scratch, entries: [["x", f64(-10_000 + i)]] });
    if (offlineFirst) {
    ...
    const ok = await waitFor(
      () =>
        client.status().unacked === 0 &&
        client.status().unsaved === 0 &&
        client.status().state === "live",
      60_000,
    );
    ...
    setResult(
      ok
        ? `${n} ops ${offlineFirst ? "queued offline and " : ""}committed in ${ms(total)} ms · ${(n / (total / 1000)).toFixed(0)} ops/s · ${newSamples} latency samples`
        : "timed out",
    );
    setRunning(null);
  };
  ...
  return (
    <section data-testid="bench">
      ...
      <div className="stats">
        <div className="stat">
          <span className="v" data-testid="bench-latency">
            {ms(s?.commitLatency.p50)}
            <span className="faint"> / {ms(s?.commitLatency.p95)}</span>
          </span>
          <span className="l">commit latency p50 / p95 (ms)</span>
        </div>
        <div className="stat">
          <span className="v">{s ? s.opsInPerSec.toFixed(1) : "—"}</span>
          <span className="l">ops/s committed (5 s window)</span>
        </div>
        <div className="stat">
          <span className="v">{ms(s?.reconnect.last)}</span>
          <span className="l">last reconnect (ms)</span>
        </div>
        <div className="stat">
          <span className="v">{ms(s?.drain.last)}</span>
          <span className="l">last queue drain (ms)</span>
        </div>
      </div>
      <div className="row">
        <button
          ...
          onClick={() => void burst(200, false)}
        >
          Run burst (200 ops)
        </button>
        <button
          className="btn sm"
          data-testid="bench-offline-burst"
          disabled={running !== null}
          onClick={() => void burst(100, true)}
        >
          Offline burst + reconnect
        </button>
      </div>
      {(running || result) && (
        <span className="faint" data-testid="bench-result">
          {running ? `running ${running}…` : result}
        </span>
      )}
    </section>
  );
