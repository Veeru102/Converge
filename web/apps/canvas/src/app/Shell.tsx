...
export function Shell({ session }: { session: Session }) {
  ...
  const style: Style = useMemo(
    () => ({ fill: 0xc7d2fe, stroke: 0x4f6ef7, strokeWidth: 0, opacity: 1 }),
    [],
  );
  const others = useMemo(
    () => presence.filter((p) => p.replica !== status.replica),
    [presence, status.replica],
  );
  ...
  const keys = useMemo(
    () => ({
      ...
      "mod+]": () => reorder(client, selectedIds, "forward"),
      "mod+[": () => reorder(client, selectedIds, "backward"),
      "mod+shift+]": () => reorder(client, selectedIds, "front"),
      "mod+shift+[": () => reorder(client, selectedIds, "back"),
      "mod+shift+}": () => reorder(client, selectedIds, "front"),
      "mod+shift+{": () => reorder(client, selectedIds, "back"),
      arrowleft: () => nudge(client, selected, -1, 0),
      arrowright: () => nudge(client, selected, 1, 0),
      arrowup: () => nudge(client, selected, 0, -1),
      arrowdown: () => nudge(client, selected, 0, 1),
      "shift+arrowleft": () => nudge(client, selected, -10, 0),
      "shift+arrowright": () => nudge(client, selected, 10, 0),
      "shift+arrowup": () => nudge(client, selected, 0, -10),
      "shift+arrowdown": () => nudge(client, selected, 0, 10),
      "mod+0": () => setViewport({ x: 0, y: 0, scale: 1 }),
      "mod+1": fit,
      "mod+=": () => zoomBy(1.2),
      "mod+-": () => zoomBy(1 / 1.2),
      "mod+.": () => setLabOpen((v) => !v),
      ...
      "shift+?": () => setSheet((v) => !v),
      enter: () => {
        if (selected.length === 1 && selected[0]![1].kind === "text") setEditingId(selected[0]![0]);
      },
    }),
  ...
  return (
    <div className="shell">
      <div className="topbar">
        ...
        <span className="doc" title={docId}>
          {docId.slice(0, 8)}
        </span>
        ...
        <button
          className={`icon-btn${labOpen ? " active" : ""}`}
          title="Network Lab (⌘.)"
      ...
      <div className="stage" ref={stageRef}>
        ...
        {objects.length === 0 && tool === "select" && (
          <div className="hint" data-testid="empty-hint">
            <h2>Blank canvas</h2>
            <div>
              Press <kbd>R</kbd> for a rectangle, <kbd>T</kbd> for text, or share this URL to
              collaborate.
            </div>
          </div>
        )}
